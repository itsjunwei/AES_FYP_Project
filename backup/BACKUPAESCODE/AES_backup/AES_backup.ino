...
void setup() {
  ...
  //Nr-The number of roud encrypted (AES-128 10 rounds, AES-192 12 rounds, AES-256 14 rounds) 
  //Nk-The number of 32-bit words contained in the key (AES-128 4, AES-192 6, AES-256 8)
  const int Nr = 10;  //10,12,14
  const int Nk = 4;   //4,6,8  
  ...
  //Round constant, used in key expansion. (AES-128 takes 10 rounds)  
...
/*****************************************Encryption Process *********************************************/  
//Performing the encryption of the plaintext block using 4 different trnasformation in the initial round, 
...
void encrypt(byte in[4*4], uint32_t w[4*(10+1)])  // Change Nr for AES-256
//  Nr = 10,12,14 
//  Nk = 4,6,8
{  
    uint32_t key[4];  
...
// void loop() {           // without triggering via pySerial, encrytpion only               
...
void loop() {           // triggering via pySerial

  if (Serial.available() > 0) {
  incomingByte = Serial.readStringUntil('\n');

    if (incomingByte == "on") {
      digitalWrite(LED_BUILTIN, HIGH);            // Encryption starts on LED
      digitalWrite(11, HIGH);                     // Encryption starts pin11 HIGH

      uint32_t w[4*(Nr+1)];  
      KeyExpansion(key, w);                       // Key expansion
    
      encrypt(plain, w);                          // Perform Encryption
      Serial.print("Encrypted ciphertext:\n");    // Print ciphertext     
      for(int i=0; i<16; ++i){                    // Print ciphertext  
          Serial.print(plain[i],HEX);             // Print ciphertext  
          if((i+1)%4 == 0)                        // Print ciphertext  
              Serial.print("\n");                 // Print ciphertext  
      }                                           // Print ciphertext  
      Serial.print("\n");                         // Print ciphertext  

      Serial.println(analogRead(A0));             // Write to dataFile.txt
      delay(1000);
      digitalWrite(LED_BUILTIN, LOW);             // Encryption ends off LED
      digitalWrite(11, LOW);                      // Encryption ends pin11 LOW
    }
